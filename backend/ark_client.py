...
IMAGE_EXTRACTION_PROMPT = """You are a precision data extraction tool. Your task is to analyze the provided image of a temperature profile graph and extract the data points represented by the markers on the line.
...
Do not wrap the output in markdown code blocks. Output ONLY the raw comma-separated text."""


LOG_SIMULATION_PROMPT = """You are a medical device transport log simulator. Generate realistic transport log entries based on the provided temperature profile CSV data.
...
class ArkClient:
    def __init__(self, api_key, base_url, vision_model, text_model=None):
        self.api_key = api_key
        self.base_url = base_url.rstrip('/')
        self.vision_model = vision_model
        self.text_model = text_model or vision_model
    ...
    def analyze_image_as_csv(self, image_path, mime_type='image/png'):
        ...
        if not self.vision_model:
            raise ValueError('ARK_VISION_MODEL is not set. Set ARK_VISION_MODEL in backend/.env')
    ...
    def generate_simulated_logs(self, csv_data):
        ...
        if not self.text_model:
            raise ValueError('ARK_TEXT_MODEL is not set. Set ARK_TEXT_MODEL in backend/.env')

        prompt = LOG_SIMULATION_PROMPT.format(csv_data=csv_data)
